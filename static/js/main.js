...
function updateRecordingUI(isRecording) {
    const statusMessage = document.getElementById('recordingStatus');
    ...
    try {
        ...
        if ('webkitSpeechRecognition' in window) {
            ...
            recognition.onerror = (event) => {
                ...
                handleRecognitionError(event.error, isBrave);
                isRecording = false;
                updateRecordingUI(false);
            };
...
function handleRecognitionError(error, isBrave) {
    let errorMessage = '';
    switch(error) {
        case 'network':
            errorMessage = isBrave ? 
                'Network error: Please check your Brave browser settings. Click the Brave shield icon (lion) in the address bar and allow microphone access.' :
        ...
        case 'audio-capture':
            errorMessage = isBrave ?
                'No microphone found. Please check your Brave browser settings and allow microphone access.' :
                'No microphone found. Please check your microphone connection';
            break;
        case 'not-allowed':
            errorMessage = isBrave ?
                'Microphone access denied. Please click the Brave shield icon (lion) in the address bar and allow microphone access.' :
        ...
        case 'service-not-available':
            ...
            break;
        default:
            errorMessage = 'Error with speech recognition: ' + error;
    }
...
// Function to convert text amount to number
function convertTextToNumber(text, language) {
    ...
    // Split the text into words and handle numbers with words (e.g., "1lakh")
    const words = text.toLowerCase()
        .replace(/(\d+)(lakh|thousand|hundred|crore)/g, '$1 $2') // Add space between number and word
        .split(/\s+/);
    ...
    for (let word of words) {
        ...
        if (num) {
            if (num >= 1000) { // For thousands, lakhs, crores
                if (currentNumber === 0) currentNumber = 1;
                currentNumber *= num;
                result += currentNumber;
                currentNumber = 0;
            } else if (num >= 100) { // For hundreds
                if (currentNumber === 0) currentNumber = 1;
                currentNumber *= num;
            } else { // For single digits
                currentNumber += num;
            }
        } else {
    ...
    // Add any remaining currentNumber to result
    result += currentNumber;
...
function handleDepositVoiceInput(field, transcript) {
    ...
    // Handle denomination details
    if (field.id === 'denominationDetails') {
        // Extract numbers from transcript
        const numbers = transcript.match(/\d+/g);
        if (numbers && numbers.length > 0) {
            // Get the current denomination field being filled
            const denominationFields = [
                'd2000', 'd500', 'd200', 'd100', 'd50', 'd20', 'd10'
            ];
            
            // Find the first empty or zero value field
            let targetField = null;
            for (const fieldId of denominationFields) {
            ...
            if (targetField) {
                // Set the value to the first number found
                targetField.value = numbers[0];
                // Trigger input event to update total
                targetField.dispatchEvent(new Event('input'));
                
                // Focus the next denomination field
                const currentIndex = denominationFields.indexOf(targetField.id);
...
// Setup click handlers for all form fields
    ...
    forms.forEach(form => {
        ...
        fields.forEach(field => {
            field.addEventListener('focus', () => {
                currentField = field;
                lastFocusedField = field; // Store the focused field
                updateVoiceStatus();
            });
        });
    });
